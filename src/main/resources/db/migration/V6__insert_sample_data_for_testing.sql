...
-- This migration will only run in dev/test environments

DO $$
BEGIN
    -- Only insert sample data if no articles exist (fresh installation)
    IF NOT EXISTS (SELECT 1 FROM articles LIMIT 1) THEN
        ...
        ('hankyung', 'https://example.com/article1', '한국은행 기준금리 동결...경기 회복세 지속', 
         CURRENT_TIMESTAMP - INTERVAL '1 day', '김기자', 
        ...
        (1, 'gpt-4o-mini', 
         ...
         ARRAY['기준금리 동결 결정', '경기 회복세 지속 판단', '금융시장 안정성 고려'],
         '[{"term": "기준금리", "definition": "중앙은행이 시중은행에 돈을 빌려줄 때 적용하는 기본 금리"}]'::JSONB,
         85.5),
        
        (2, 'gpt-4o-mini',
         '코스피가 2400선을 회복하며 상승세를 보였습니다. 외국인 투자자들의 지속적인 매수가 주요 동력이었습니다.',
         '코스피 상승은 국내 기업들의 시가총액 증가와 투자심리 개선을 의미합니다.',
         ARRAY['코스피 2400선 회복', '외국인 매수세 지속', '투자심리 개선'],
         '[{"term": "코스피", "definition": "한국종합주가지수로 국내 주식시장의 대표 지수"}]'::JSONB,
         78.2);
        ...
        RAISE NOTICE 'Sample data inserted successfully for testing environment';
    ELSE
        RAISE NOTICE 'Sample data insertion skipped - articles table already contains data';
    END IF;
END $$;