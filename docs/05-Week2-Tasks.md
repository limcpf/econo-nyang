...
#### 수락 기준:
- [ ] 배치 실행 시 ExecutionContext 직렬화 오류 없음
- [ ] Step 간 데이터 전달 정상 동작
...
#### 수락 기준:
- [ ] Discord 채널에 메시지 발송 성공
- [ ] 긴 메시지 자동 분할 동작
- [ ] Rate Limit 상황 재시도 동작
- [ ] 발송 성공/실패 로그 기록
- [ ] DRY RUN 모드 동작
...
#### 수락 기준:
- [ ] 템플릿 기반 마크다운 다이제스트 생성
- [ ] 변수 치환 정상 동작
- [ ] 다양한 템플릿 형식 지원
- [ ] DailyDigest DB 저장 성공
...
#### 수락 기준:
- [ ] OpenAI API 호출 및 응답 처리 성공
- [ ] Structured Outputs JSON 스키마 준수
- [ ] 중요도 점수 정확도 검증
- [ ] API 비용 모니터링 구현
