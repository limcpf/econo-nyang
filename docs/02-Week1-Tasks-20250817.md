...
### **Task 2: PostgreSQL 데이터베이스 스키마 설계 및 Flyway 마이그레이션 파일 작성**
**우선순위**: 최고  
**예상 소요시간**: 3시간
...
- Flyway 설정 및 의존성 추가
- DB 마이그레이션 파일 작성:
  - `V1__articles.sql` - 기사 정보 테이블
  - `V2__summaries.sql` - AI 요약/해설 테이블  
  - `V3__daily_digest.sql` - 일일 다이제스트 테이블
  - `V4__dispatch_log.sql` - 발송 로그 테이블
- PostgreSQL 연결 설정 및 테스트
...
### **Task 3: Spring Batch 기본 설정 및 ECON_DAILY_DIGEST Job 구조 구현**
**우선순위**: 최고  
**예상 소요시간**: 4시간

#### 상세 작업
- Spring Batch 설정 클래스 구현
- `ECON_DAILY_DIGEST` Job 정의
- Job Parameters 설정 (targetDate, maxArticles, dryRun, useLLM 등)
- 5개 Step의 기본 구조 정의:
